...
void CClothSimulationTask::ComputeGPU(cl_context , cl_command_queue CommandQueue, size_t LocalWorkSize[3])
{
	...
	V_RETURN_CL(clErr, "Error executing the integration kernel");

	// Check for collisions
	clErr = clSetKernelArg(m_CollisionsKernel, 0, sizeof(unsigned int), &m_ClothResX);
	clErr |= clSetKernelArg(m_CollisionsKernel, 1, sizeof(unsigned int), &m_ClothResY);
	...
	for (unsigned int i = 0; i < 2 * m_ClothResX; i++)
	{
		...
		V_RETURN_CL(clErr, "Failed to set the constraint kernel params");
		...
		V_RETURN_CL(clErr, "Error executing the constraint kernel");

		//	 Swap the ping pong buffers
		std::swap(m_clPosArray, m_clPosArrayAux);
